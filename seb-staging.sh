...
if [[ -z "$CONTEST_CODE" ]]; then
  echo "ERROR: Contest code not specified." >&2
  exit 1
fi
...
# ---- 2. Determine installed vs. pinned version ----
installed_version=""
if [[ -d "$APP_PATH" ]]; then
...
if [[ -n "$installed_version" ]] && version_ge "$installed_version" "$FALLBACK_MAC_VERSION"; then
  needs_install=false
  log "Installed SEB $installed_version meets the pinned version $FALLBACK_MAC_VERSION - skipping install."
else
  log "Installed SEB version: ${installed_version:-none} is older than the pinned version $FALLBACK_MAC_VERSION. Installing pinned version."
fi
...
if $needs_install; then
  ...
  log "Downloading Safe Exam Browser $FALLBACK_MAC_VERSION..."
  DMG_PATH="$TMP_DIR/SafeExamBrowser.dmg"
  curl -fsSL "$FALLBACK_MAC_DMG_URL" -o "$DMG_PATH"
  ...
  log "Safe Exam Browser $FALLBACK_MAC_VERSION installed."
fi
